...
import com.github.thwak.confix.tester.coverage.CoveredLine;
...
public class PatchStrategy {
    ...
    protected Map<String, PatchApplier> patcherMap;
        ...
        if (currLocIndex < locations.size()) {
            ...
            if (++currLineIndex < coveredLines.size()) {
                ...
                if (!patcherMap.containsKey(cl.className)) {
                    ...
                    ConcretizationStrategy cStrategy = StrategyGenerator.getConcretizationStrategy(cStrategyKey, manager,
                            cl.className, sourceDir, r);
                    PatchApplier patchApplier = new PatchApplier(cl.className, source, compileClassPathEntries,
                            new String[]{sourceDir}, this, cStrategy);
                    patcherMap.put(cl.className, patchApplier);
                }
        ...
        while (it.hasNext()) {
            ...
            if (loc.kind != TargetLocation.DEFAULT && checkDescriptor(loc, c) && loc.isCompatible(c)) {
                ...
                if (c.node.hashString == null) {
                    c.node.hashString = DiffUtils.getTypeHash(c.node);
                }
