package com.github.thwak.confix.pool.utils;
...
import com.github.thwak.confix.pool.models.Replace;
...
public class Converter {
    ...
    public static Script convert(String id, EditScript editScript, String oldCode, String newCode, boolean normalize) {
        ...
        for (EditOp op : editScript.getEditOps()) {
            ...
            if (op instanceof Delete) {
                List<Node> traverse = DiffUtils.traverse(oldRoot);
                for (Node n : traverse) {
                    if (n.id == nodeId) {
                        node = n;
                ...
                List<Node> traverse = DiffUtils.traverse(newRoot);
                for (Node n : traverse) {
                    if (n.id == nodeId) {
                        node = n;
                    } else if (n.id == locationId) {
                ...
                traverse = DiffUtils.traverse(newRoot);
                for (Node n : traverse) {
                    if (n.id == locationId) {
                        location = n;
                        break;
            ...
            if (op instanceof Move) {
                ...
                location = DiffUtils.deepCopy(location);
            }

            if (normalize) {
                if (op instanceof Update || op instanceof Replace) {
                    DiffUtils.normalize(node, false);
                    DiffUtils.normalize(manager, location, false);
                } else {
                    DiffUtils.normalize(manager, node, false);
                    DiffUtils.normalize(location, false);
                }
            }
    ...
    private static Node getCopiedNode(Node node) {
        ...
        Node copied = DiffUtils.deepCopy(node);
        if (node.parent != null) {
    ...
    private static void updateEdit(MVTManager manager, Change change, Node node, String source) {
        ...
        List<Node> nodes = DiffUtils.traverse(node);
        ...
        for (Node n : nodes) {
            ...
            if (n.normalized) {
                ...
                sb.append(code, lastAppendedIndex, startIndex);
                sb.append(n.value);
    ...
    public static Node convert(TreeNode node) {
        ...
        Node n = DiffUtils.getNode(astNode);
        n.id = node.getId();
