...
import com.github.thwak.confix.patch.models.Materials;
...
import com.github.thwak.confix.pool.ingredients.MVTManager;
import com.github.thwak.confix.pool.ingredients.Method;
import com.github.thwak.confix.pool.ingredients.Variable;
import com.github.thwak.confix.pool.ingredients.VariableType;
import com.github.thwak.confix.pool.models.Requirements;
...
public class ConcretizationStrategy {
    ...
    public boolean resolveType(TargetLocation loc) {
        ...
        if (astNode instanceof Expression && loc.kind == TargetLocation.DEFAULT) {
    ...
    public void collectMaterials(Node root) {
        ...
        List<Node> nodes = DiffUtils.traverse(root);
        for (Node n : nodes) {
                    ...
                    if (!isLocal) {
                        ...
                        while (name.isQualifiedName()) {
                            ...
                            if (name.resolveBinding() != null) {
                                qNode.value = DiffUtils.getValue(name);
                                qNode.astNode = name;
    ...
    public boolean instCheck(Change change, TargetLocation loc) {
        ...
        if (change.type.equals(Change.UPDATE) && change.node.type == loc.node.type && change.node.children.size() > 0) {
            if (change.node.hashString == null) {
                change.node.hashString = DiffUtils.getTypeHash(change.node);
            }
            if (loc.node.hashString == null) {
                loc.node.hashString = DiffUtils.getTypeHash(loc.node);
            }
    ...
    public ASTNode instantiate(Change c, TargetLocation loc, PatchInfo info) {
        ...
        if (c.type.equals(Change.REPLACE)) {
            ...
            List<Node> locNodes = DiffUtils.traverse(loc.node);
            List<Node> cNodes = DiffUtils.traverse(c.node);
            ...
            for (String oldValue : reqs.declaredVariables) {
                ...
                while (materials.varNames.contains(newValue)) {
                    newValue = DiffUtils.VAR_PREFIX + count++;
                }
        ...
        astNode = DiffUtils.generateNode(copied, ast);
    ...
    protected ASTNode updateIntermediate(Change c, TargetLocation loc) {
        ...
        if (c.location.label.contains("fixExpression")) {
            astNode = DiffUtils.generateNode(copied, ast, loc.node);
        } else {
            astNode = DiffUtils.generateNode(copied, ast);
        }
        return astNode;
