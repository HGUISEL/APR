...
public class ContextIdentifier implements Serializable {
    ...
    protected void addAncestors(Node node, String desc, StringBuffer sb, int level) {
        sb.append(SYM_OPEN);
        sb.append(DiffUtils.getTypeName(node.type));
        sb.append(DiffUtils.SYM_OPEN);
        sb.append(desc);
        sb.append(DiffUtils.SYM_CLOSE);
        sb.append(SYM_CLOSE);
        Node parent = node;
        for (int i = 1; i < level; i++) {
            ...
            if (parent == null) {
                ...
                sb.append(DiffUtils.getTypeName(parent.type));
                if (node.desc != null) {
                    sb.append(DiffUtils.SYM_OPEN);
    ...
    protected String getParentType(Node node, Node parent) {
        if (parent == null) {
            ...
            if (parent.type == ASTNode.SWITCH_STATEMENT
                ...
                return DiffUtils.getTypeName(ASTNode.SWITCH_CASE);
            } else {
                return DiffUtils.getTypeName(parent.type);
            }
        }
    }
}
